...
from .forms import CustomUserCreationForm, JobApplicationForm, ResumeForm, JobForm
...
def job_applications_list(request):
    """
    Employers and Admins can see all job applications.
    """
...
def extract_text_from_resume(resume_file):
    """
    Resume faylidan matnni chiqarib olish.
...
def analyze_resume(resume_text):
    """
    Resume matnini AI yordamida tahlil qiladi va 15 texnik + 5 psixologik intervyu savollarini yaratadi.
    """
    if not resume_text.strip():
        return ["Error: Resume content is empty or could not be extracted."]

    prompt = f"""
    You are an AI HR assistant. Analyze the following resume and generate 15 interview questions 
    related to the candidate's skills and experience, and 5 general psychological questions for the interview.
    ...
    try:
        ...
        return [q.strip() for q in questions if q.strip()]  # Bo‘sh satrlarni olib tashlash

    except Exception as e:
        return [f"Error generating questions. Please try again. (Error: {str(e)})"]


def ai_resume_review(request, application_id):
    """
    Resume faylini OpenAI orqali tahlil qilish va natijalarni sahifaga chiqarish.
    """
    application = get_object_or_404(JobApplication, id=application_id)
    ...
    # Resume matnini olish
    resume_content = extract_text_from_resume(application.resume)

    # AI orqali resume tahlil qilish
    analysis_result = analyze_resume(resume_content)

    return render(request, 'jobs/ai_review.html', {
        'application': application,
        'ai_result': "\n".join(analysis_result),  # Sahifada matn ko‘rinishida chiqadi
        'questions': analysis_result  # Intervyu savollari ro‘yxat ko‘rinishida bo‘ladi
    })
...
def edit_profile(request):
    if request.method == "POST":
        ...
        return redirect("job_seeker_dashboard")  # Redirect back to the dashboard

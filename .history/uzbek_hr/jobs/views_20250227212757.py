...
from .utils.ai_resume_analysis import analyze_resume, extract_text_from_resume
...
def ai_resume_review(request, application_id):
    ...
    """
    # Arizani olish (Application modeli orqali)
    application = get_object_or_404(Application, id=application_id)

    # Rezyume borligini tekshirish
    ...
    # Rezyume matnini olish
    resume_text = extract_text_from_resume(application.resume)

    # Ish tavsifini olish
    job = application.job  # Application modeli Job bilan bog‘langan bo‘lishi kerak
    job_description = job.description if job else "No job description provided."

    # AI orqali intervyu savollarini generatsiya qilish
    questions = analyze_resume(resume_text, job_description)

    # AI tahlil natijasini saqlash
    application.ai_review = "\n".join(questions)
    application.save()

    # Nomzodga email yuborish
    send_test_notification_email(application.user.email, job.title)
...
def generate_interview_questions(job_title, resume_text):
    ...
    try:
        ...
        print("🔹 API Response:", response)  # ✅ Debug uchun
        ...
        print("❌ Unexpected Error:", str(e))  # ✅ Har qanday boshqa xatolarni tutish

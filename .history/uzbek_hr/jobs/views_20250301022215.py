...
def ai_resume_review(request, application_id):
    ...
    send_test_notification_email(application.applicant.email, job.title if job else "Unknown Job")
...
def process_resume(request):
    """
    Django view that processes a resume file and compares it with a job description.
    """
    if request.method == "POST":
        resume_text = request.POST.get("resume_text", "")  # Extracted text from resume
        job_title = request.POST.get("job_title", "")  # Job title
        job_description = request.POST.get("job_description", "")  # Job description

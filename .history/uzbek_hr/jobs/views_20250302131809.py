...
def test_dashboard(request):
    """Shortlisted bo'lgan job seeker uchun test dashboard"""
    ...
    # ✅ AI-generated questions JSON formatini tekshirish
    try:
        questions = json.loads(application.ai_generated_questions) if application.ai_generated_questions else []
    except json.JSONDecodeError:
        return render(request, "test/error.html", {"message": "Xatolik: Savollarni yuklab bo‘lmadi."})

    if not questions:
        return render(request, "test/no_questions.html", {"message": "Siz uchun savollar hali generatsiya qilinmagan."})

    # ✅ POST so‘rovlar uchun - foydalanuvchi javoblarni saqlaydi
    if request.method == "POST":
        question_index = int(request.POST.get("question_index", 0))
        answer = request.POST.get("answer", "").strip()

        # ✅ Test javoblarini tekshirish va yangilash
        if not application.test_answers:
            application.test_answers = {}
    ...
    # ✅ GET so‘rovlar uchun - hozirgi savolni olish
    question_index = int(request.GET.get("question", 0))
    if question_index >= len(questions):  # Xatolik oldini olish
        return redirect("test_complete")

    question = questions[question_index]

    return render(request, "test/test_dashboard.html", {
        "question": question,
...
def generate_interview_questions(resume_text, job_title, job_requirements):
    """AI yordamida intervyu savollarini generatsiya qiladi"""
    
    prompt = f"""
    A job application has been submitted. The applicant's resume is below:
    {resume_text}

    The job title is: {job_title}
    The job requirements are: {job_requirements}

    Generate 15 technical questions and 5 general interview questions based on the above information.
    """
    ...
    try:
        ...
        print("🔹 API Response:", response)  # Terminalda natijani tekshirish
...
def save_generated_questions(application_id):
    """Generatsiya qilingan savollarni bazaga saqlash"""
    ...
    # JSON formatga o‘tkazish va bazaga saqlash
    application.ai_generated_questions = json.dumps(questions)
    application.save()
    
    print(f"✅ Questions saved for application {application_id}")
    
    return JsonResponse({"message": "Questions generated successfully!"})

def view_generated_questions(request, application_id):
    """Job seeker dashboard uchun AI-generated intervyu savollarini ko‘rsatish"""
    
    application = get_object_or_404(JobApplication, id=application_id)

    try:
        questions = json.loads(application.ai_generated_questions)  # JSONni o‘qish
    except json.JSONDecodeError:
        questions = []  # Xatolik bo‘lsa, bo‘sh ro‘yxat qaytariladi
    
    return render(request, "job_seeker/questions.html", {"questions": questions})
...
def shortlist_dashboard(request):
    ...
    return render(request, "test/shortlist_dashboard.html", {"jobs": jobs_with_shortlisted_applicants}) 

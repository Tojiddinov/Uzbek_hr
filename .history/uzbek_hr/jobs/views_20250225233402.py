...
@login_required
def test_dashboard(request):
    application = get_object_or_404(JobApplication, user=request.user, status='shortlisted')

    # Deadline tekshirish
    if application.test_deadline and now() > application.test_deadline:
        return render(request, 'job_seeker/test_expired.html')
    ...
    return render(request, "job_seeker/test_dashboard.html", {"question": question, "question_index": question_index}) 
# end invitation_link 
# job create end
def home(request):

...
def test_dashboard(request):
    user = request.user
    application = get_object_or_404(JobApplication, applicant=user)

    # TestDashboard obyektini yaratish (agar mavjud bo‘lmasa)
    test_dashboard, created = TestDashboard.objects.get_or_create(application=application)

    # Deadline tekshirish
    if application.test_deadline and now() > application.test_deadline:
        return render(request, 'job/test_expired.html')
    ...
    if request.method == "POST":
        ...
        if question_index + 1 < len(application.ai_generated_questions):
            return redirect(f"/test_dashboard/?question={question_index + 1}")
        else:
...
def shortlist_application(request, application_id):
    ...
    # ✅ Resume-ni o'qib olish (faylni stringga o'zgartirish)
    resume_text = "Resume is attached but parsing not implemented."  # Keyin AI-based parsing qo'shish mumkin
    
    # ✅ AI-generated interview questions
    application.ai_generated_questions = generate_interview_questions(application.job.title, resume_text)
